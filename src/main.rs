...
fn output_policy(
    ...
    Ok(match mode {
        Mode::Auto => {
            ...
            let stask = Rc::new(RefCell::new(policy.to_stask(&username, task.as_deref())));
            ...
            stask.as_ref().borrow_mut().options = Some(rc_refcell!(options));
            if let Some(task) = task {
                stask.as_ref().borrow_mut().purpose = Some(format!("{} (Generated by rootasrole)", task));
            }
            if let Some(config_path) = config {
                    ...
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
                {
                    ...
                    if let Some(role) = conf.role(&role_name) {
                        if role
                            .as_ref()
                            .borrow_mut()
                            .tasks
