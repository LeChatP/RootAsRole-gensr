...
fn output_policy(
    ...
    Ok(match mode {
        Mode::Auto => {
            let role_name = role.clone().unwrap_or(username.to_string());
            ...
            if let Some(config_path) = config {
                let fullsettings = if Path::new(&config_path).exists()
                    && let Ok(fullsettings) = rootasrole_core::read_full_settings(&config_path)
                {
                    debug!(
                        "Updating existing configuration file at {}",
                        config_path
                    );
                    let fullsettings_ref = fullsettings.as_ref().borrow();
                    let config = fullsettings_ref.config.as_ref().ok_or_else(|| {
                        io::Error::new(io::ErrorKind::Other, "No config found in settings")
                    })?;
                        ...
                        .find(|r| r.as_ref().borrow().name == role_name)
                    {
                        ...
                        debug!(
                            "Creating new role '{}' in configuration",
                            role_name
                        );
                        let role = SRole::builder(role_name.clone())
                            .extra_fields(
                                json!({"purpose" : role.clone()}).as_object().cloned().unwrap(),
                            )
                            .task(stask.clone())
                            .build();
                ...
                let file = std::fs::File::create(&config_path)
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
                serde_json::to_writer_pretty(&file, &Versioning::new(fullsettings))
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
                file.sync_all()
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
...
fn prompt_for_confirmation(yes: bool, config: &str) -> Result<(), io::Error> {
    ...
    println!(
        "This will deploy rootasrole config ({}) to the system, are you sure you want to continue? [Y/n]",
