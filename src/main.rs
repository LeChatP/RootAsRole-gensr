...
fn main() -> io::Result<()> {
    ...
    match args.command {
        ...
        Commands::Generate {
            ...
            output_policy(mode, config, role, task, username, policy)
        }
...
fn output_policy(
    ...
    Ok(match mode {
        Mode::Auto => {
            let role_name = role.unwrap_or(username.clone());
            ...
            if let Some(config_path) = config {
                let fullsettings = rootasrole_core::read_full_settings(&config_path)
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
                    ...
                    .find(|r| r.as_ref().borrow().name == role_name)
                {
                    ...
                    let role = SRole::builder(role_name.clone())
                        .extra_fields(json!({"purpose" : role_name}).as_object().cloned().unwrap())
                        .task(stask.clone())
                        .build();
                ...
                println!("{}", serde_json::to_string_pretty(&stask).unwrap());
            }
        }
        Mode::Manual => {
